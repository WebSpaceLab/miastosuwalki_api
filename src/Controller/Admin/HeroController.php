<?php

namespace App\Controller\Admin;

use App\Controller\AbstractAPIController;
use App\Entity\Hero;
use App\Repository\HeroRepository;
use App\Repository\MediaRepository;
use App\Service\HeroHelper;
use App\Service\PaginationHelper;
use App\Service\QueryHelper;
use Symfony\Component\HttpFoundation\JsonResponse;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Annotation\Route;
use Symfony\Component\Security\Http\Attribute\IsGranted;
use Symfony\Component\Validator\Constraints as Assert;
use Symfony\Component\Validator\Constraints\Length;
use Symfony\Component\Validator\Constraints\NotBlank;
use Symfony\Component\Validator\Validator\ValidatorInterface;

#[IsGranted('ROLE_ADMIN')]
#[Route('/api/admin/hero', name: 'app_admin_hero')]
class HeroController extends AbstractAPIController
{
    public function __construct(
        private PaginationHelper $paginationHelper, 
        private QueryHelper $QueryHelper,
        private HeroHelper $heroHelper
    ) {}

    #[Route('', name: ':list', methods: ['GET'])]
    public function list(Request $request, HeroRepository $heroRepository): JsonResponse
    {
        $query = $request->query->all();

        $queryBuilder = $heroRepository->getWithSearchQueryBuilder(
            $query['term'], $query['orderBy'], $query['orderDir'], $query['status'], $query['month']
        );

        $pagination = $this->paginationHelper->paginate($queryBuilder, $query['page'], $query['per_page']);

        return $this->response([
            'hero' => $pagination['data'],
            'pagination' => $pagination['pagination'],
            'queryParams' =>  $this->QueryHelper->params($request, ['term','status','month']),
            'status' => $this->heroHelper->getActive(),
            'months' => $this->heroHelper->getMonths(),
        ], ['admin:hero:read']);
    }

    #[Route('', name: ':create', methods: ['POST'])]
    public function create(Request $request, ValidatorInterface $validator,  HeroRepository $heroRepository, MediaRepository $mediaRepository): JsonResponse
    {
        $data = $request->toArray();

        $constraints = new Assert\Collection([
            'name' => [
                new NotBlank(),
                new Length(['min' => 2, 'minMessage' => 'Nazwa musi składać się z przynajmniej 2 liter.']),
            ],
            'description' => [
                new NotBlank(),
                new Length(['min' => 20, 'minMessage' => 'Nazwa musi składać się z przynajmniej 20 liter.']),
            ],
            'isActive' => [],
            'mediaId' => [
                new NotBlank(),
            ],
        ]);

        $violations = $validator->validate($data, $constraints);

        if (count($violations) > 0) {
            $errors = [];
            foreach ($violations as $violation) {
                $propertyPath = trim($violation->getPropertyPath(), '[\]');
                $errors[$propertyPath] = $violation->getMessage();
            }

            return $this->json(['errors' => $errors], Response::HTTP_BAD_REQUEST);
        }

        // return $this->json(['data' => $data], Response::HTTP_OK);
        if($heroRepository->findBy(['name' => $data['name']])) {
            return $this->json(['errors' => ['name' => 'Taka nazwa już istnieje.']], Response::HTTP_BAD_REQUEST);
        }

        $media = $mediaRepository->find($data['mediaId']);

        if(!$media) {
            return $this->json(['errors' => ['mediaId' => 'Nie znaleziono zdjęcia.']], Response::HTTP_BAD_REQUEST);
        }

        $hero = new Hero();
        
        $hero->setName($data['name']);
        $hero->setDescription($data['description']);
        $hero->setIsActive($data['isActive']);
        $hero->setMedia($media);
        $hero->setAuthor($this->getUser());
        
        try {
            $heroRepository->save($hero, true);

            $media->setIsUsed(true);
            $mediaRepository->save($media, true);
        } catch (\Throwable $th) {
            //throw $th;
            return $this->json(['flash' => [
                    'message' => $th,
                    'type' => 'error'
                ]
            ], Response::HTTP_BAD_REQUEST);
        }

        $this->flash('Hero został dodany.');

        return $this->response([
            'data' => $hero,
        ], ['admin:hero:read']);
    }

    #[Route('/{id}', name: ':update', methods: ['PATCH'])]
    public function update(Hero $hero, Request $request, ValidatorInterface $validator,  HeroRepository $heroRepository): JsonResponse
    {
        $data = $request->toArray();

        $constraints = new Assert\Collection([
            'name' => [
                new NotBlank(),
                new Length(['min' => 2, 'minMessage' => 'Nazwa musi składać się z przynajmniej 2 liter.']),
            ],
            'description' => [
                new NotBlank(),
                new Length(['min' => 20, 'minMessage' => 'Nazwa musi składać się z przynajmniej 20 liter.']),
            ],
            'isActive' => [],
            'mediaId' => [
                new NotBlank(),
            ],
        ]);

        $violations = $validator->validate($data, $constraints);

        if (count($violations) > 0) {
            $errors = [];
            foreach ($violations as $violation) {
                $propertyPath = trim($violation->getPropertyPath(), '[\]');
                $errors[$propertyPath] = $violation->getMessage();
            }

            return $this->json(['errors' => $errors], Response::HTTP_BAD_REQUEST);
        }

        $hero->setName($data['name']);
        $hero->setDescription($data['description']);
        $hero->setIsActive($data['isActive']);
        
        $heroRepository->save($hero, true);

        $this->flash('Zmiana została zapisana.');

        return $this->response([
            'hero' => $hero,
        ], ['admin:hero:read']);
    }

    #[Route('/{id}', name: ':delete', methods: ['DELETE'])]
    public function remove(Hero $hero, HeroRepository $heroRepository): JsonResponse
    {
        if (!$hero) {
            return $this->json([
                'flash' => [
                    'type' => 'error',
                    'message' => 'Wiadomość nie istnieje.',
                ],
            ]);
        }

        $hero->setIsDelete(true);

        $heroRepository->save($hero, true);

        return $this->json([
            'flash' => [
                'type' => 'success',
                'message' => 'Wiadomość została usunięta.',
            ],
        ], Response::HTTP_OK);
    }
}
